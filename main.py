...
<body>
    ...
    <section class="hero">
        ...
        <p>Professional access control system with device binding, expiration management, and real-time monitoring. Secure your mod menu with enterprise-grade key authentication.</p>
    ...
    <section class="features" id="features">
        ...
        <div class="features-grid">
            <div class="feature-card">
                ...
                <p>Each key is locked to a single device. Cannot be shared or transferred to another phone.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">⏱️</div>
                <h3>Time-Limited Access</h3>
                <p>Flexible duration options from 1 hour to lifetime. Automatic expiration enforcement.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">📊</div>
                <h3>Real-Time Analytics</h3>
                <p>Monitor key usage, active devices, and access patterns from the admin dashboard.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">🚫</div>
                <h3>Ban System</h3>
                <p>Instantly ban keys, devices, or IP addresses. Prevent unauthorized access attempts.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">🔄</div>
                <h3>Auto-Check</h3>
                <p>Periodic validation ensures keys remain valid during gameplay sessions.</p>
            </div>
            <div class="feature-card">
                <div class="feature-icon">📱</div>
                <h3>Android Integration</h3>
                <p>Seamless integration with Sketchware Pro projects. Easy copy-paste setup.</p>
            </div>
        </div>
    </section>
    ...
    <section class="pricing" id="pricing">
        ...
        <div class="pricing-grid">
            <div class="pricing-card">
                <h3>1 Hour Trial</h3>
                <div class="price">$0.50 <span>/hour</span></div>
                <ul>
            ...
            <div class="pricing-card">
                <h3>1 Day Pass</h3>
                <div class="price">$2.00 <span>/day</span></div>
                <ul>
            ...
            <div class="pricing-card popular">
                ...
                <h3>1 Week</h3>
                <div class="price">$8.00 <span>/week</span></div>
                <ul>
                    <li>7 days access</li>
                    <li>Single device</li>
            ...
            <div class="pricing-card">
                <h3>1 Month</h3>
                <div class="price">$25.00 <span>/month</span></div>
                <ul>
            ...
            <div class="pricing-card" style="border-color: var(--success);">
                <h3>Lifetime</h3>
                <div class="price" style="color: var(--success);">$50.00 <span>/once</span></div>
                <ul>
    ...
    <div class="container">
        ...
        <div class="card">
            ...
            <div class="warning">
                ...
                Your key will be permanently bound to this device. Cannot be transferred.
            </div>
        </div>
            ...
            <div id="tab-keys" class="tab-content">
                <div class="card">
                    <div class="card-header">
                        ...
                        <input type="text" placeholder="Search keys..." id="keySearch"
                               onkeyup="searchKeys()">
                    </div>
                    <div class="card-body">
                            ...
                            <table id="keysTable">
                                ...
                                <tbody id="keysTableBody">
                                    ...
                                    <tr data-key="{{ key }}">
                                        ...
                                        <td>
                                            {% if data.device_id %}
                                            <span class="device-text" title="{{ data.device_id }}">{{ data.device_id[:12] }}...</span>
                                            {% else %}
                                        ...
                                        <td style="color: var(--text-secondary); font-size: 12px;">
                                            ...
                                            {% else %}
                                            {{ data.expires | timestamp_to_date }}
                                            {% endif %}
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            {% elif data.expires != 9999999999 and data.expires < now %}
                                            <span class="badge badge-danger"><i class="fas fa-clock"></i> Expired</span>
                                            {% elif data.device_id %}
                                            <span class="badge badge-success"><i class="fas fa-check"></i> Active</span>
                                            {% else %}
                                            <span class="badge badge-warning"><i class="fas fa-unlock"></i> Unused</span>
                                            {% endif %}
                                        </td>
                                        <td>
                                            <div class="action-btns">
                                                <button class="action-btn copy" onclick="copyKey('{{ key }}')" title="Copy">
                                                    <i class="fas fa-copy"></i>
                                                </button>
                                                <button class="action-btn extend" onclick="extendKey('{{ key }}')" title="Extend +24h">
                                                    <i class="fas fa-plus"></i>
                                                </button>
                                                {% if data.banned %}
                                                <button class="action-btn ban" onclick="unbanKey('{{ key }}')" title="Unban">
                                                    <i class="fas fa-check"></i>
                                                </button>
                                                {% else %}
                                                <button class="action-btn ban" onclick="banKey('{{ key }}')" title="Ban">
                                                    <i class="fas fa-ban"></i>
                                                </button>
                                                {% endif %}
                                                <button class="action-btn delete" onclick="deleteKey('{{ key }}')" title="Delete">
    ...
    <div class="modal-overlay" id="modalOverlay">
        <div class="modal">
            ...
            <div class="modal-actions">
                ...
                <button class="btn btn-danger" id="modalConfirm" onclick="">Confirm</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        async function generateKeys() {
            ...
            if (result && result.keys) {
                ...
                list.innerHTML = result.keys.map(key => `
                    <div class="key-item">
                        <span>${key}</span>
                        <button onclick="copyToClipboard('${key}')" title="Copy"><i class="fas fa-copy"></i></button>
                    </div>
                `).join('');
        ...
        async function extendKey(key) {
            const result = await apiCall('/api/extend-key', 'POST', { key, hours: 24 });
            if (result && result.success) {
                showToast('Key extended +24h!');
                refreshData();
            }
        }

        async function banKey(key) {
            showModal('Ban Key', `Ban key ${key}?`, async () => {
                const result = await apiCall('/api/ban-key', 'POST', { key });
                if (result && result.success) {
                    showToast('Key banned!');
                    refreshData();
                }
            });
        }
        ...
        async function unbanKey(key) {
            const result = await apiCall('/api/unban-key', 'POST', { key });
            if (result && result.success) {
                showToast('Key unbanned!');
                refreshData();
            }
        }

        async function deleteKey(key) {
            showModal('Delete Key', `Permanently delete ${key}?`, async () => {
                const result = await apiCall('/api/delete-key', 'POST', { key });
                if (result && result.success) {
        ...
        async function cleanupExpired() {
            showModal('Cleanup', 'Delete all expired keys?', async () => {
                const result = await apiCall('/api/cleanup', 'POST');
                if (result) {
                    showToast(`Deleted ${result.deleted} keys!`);
                    refreshData();
                }
            });
        }
        ...
        async function loadLogs() {
            ...
            if (result && result.logs) {
                if (result.logs.length === 0) {
                    container.innerHTML = '<div class="empty-state"><i class="fas fa-inbox"></i><p>No logs</p></div>';
                    return;
                }
                container.innerHTML = result.logs.map(log => `
                    <div class="log-item">
                        <div class="log-time">${new Date(log.timestamp * 1000).toLocaleTimeString()}</div>
                        <div class="log-action ${getActionClass(log.action)}">${log.action}</div>
                        <div class="log-details">${log.details} <span style="color: var(--text-muted);">(${log.ip})</span></div>
                    </div>
                `).join('');
            }
        ...
        function getActionClass(action) {
            if (action.includes('SUCCESS') || action.includes('GENERATE')) return 'success';
            if (action.includes('FAIL') || action.includes('DELETE')) return 'fail';
            if (action.includes('BAN')) return 'warn';
            return 'info';
        }

        async function loadKeys() {
            // Keys rendered server-side
        }

        function searchKeys() {
            const query = document.getElementById('keySearch').value.toLowerCase();
            const rows = document.querySelectorAll('#keysTableBody tr');
            rows.forEach(row => {
                const key = row.getAttribute('data-key').toLowerCase();
                const device = row.querySelector('.device-text')?.textContent.toLowerCase() || '';
                row.style.display = (key.includes(query) || device.includes(query)) ? '' : 'none';
            });
        }

        function clearLogs() {
            showModal('Clear Logs', 'Delete all logs?', () => {
